...
| 属性 | 值 |
|------|-----|
| Endpoint | `GET https://app-duaapde1f1tt-api-zYkZXBBNPe8L-gateway.appmiaoda.com/api_place_pro/v1/region` |
| 原始 API scheme | 原始 APIDOC 端点为 `http://app-duaapde1f1tt-api-zYkZXBBNPe8L-gateway.appmiaoda.com/api_place_pro/v1/region`（HTTP），gateway 已将其升级为 HTTPS 代理访问，调用时统一使用上方 HTTPS 地址即可 |
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    `https://app-duaapde1f1tt-api-zYkZXBBNPe8L-gateway.appmiaoda.com/api_place_pro/v1/region?${params.toString()}`,
    {
      method: "GET",
      headers: {

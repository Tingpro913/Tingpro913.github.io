...
| Endpoint | `POST https://app-duaapde1f1tt-api-V9PworyO6gEa-gateway.appmiaoda.com/train/station2s` |
| 认证方式 | platform_managed（密钥由平台注入） |
| Auth Header | `X-Gateway-Authorization: Bearer ${apiKey}` |
| Content-Type | `application/json;charset=UTF-8` |
| Third-party Domain | `app-duaapde1f1tt-api-V9PworyO6gEa-gateway.appmiaoda.com` |
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    `https://app-duaapde1f1tt-api-V9PworyO6gEa-gateway.appmiaoda.com/train/station2s?${params}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json;charset=UTF-8",

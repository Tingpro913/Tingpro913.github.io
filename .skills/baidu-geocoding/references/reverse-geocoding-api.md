...
| Endpoint | `GET https://app-duaapde1f1tt-api-baBwZEjbe1X9-gateway.appmiaoda.com/reverse_geocoding/v3` |
| 认证模式 | `platform_managed`（`traefik: true`） |
| Auth Header | `X-Gateway-Authorization: Bearer <INTEGRATIONS_API_KEY>` |
| Content-Type | `application/x-www-form-urlencoded`（GET 请求参数附在 URL 上） |
| third_part_domain | `app-duaapde1f1tt-api-baBwZEjbe1X9-gateway.appmiaoda.com` |
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    `https://app-duaapde1f1tt-api-baBwZEjbe1X9-gateway.appmiaoda.com/reverse_geocoding/v3?${params.toString()}`,
    {
      method: "GET",
      headers: {

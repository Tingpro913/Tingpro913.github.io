...
| Endpoint | `POST https://app-duaapde1f1tt-api-k93RZBjPykEa-gateway.appmiaoda.com/v2/chat/completions` |
| 认证模式 | `platform_managed` |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| Content-Type | `application/json` |
| 响应类型 | SSE 流式（`text/event-stream`） |
| third_part_domain | `app-duaapde1f1tt-api-k93RZBjPykEa-gateway.appmiaoda.com` |
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-duaapde1f1tt-api-k93RZBjPykEa-gateway.appmiaoda.com/v2/chat/completions",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
- 前端只与 Supabase Edge Function 通信，不直接访问 `app-duaapde1f1tt-api-k93RZBjPykEa-gateway.appmiaoda.com`
